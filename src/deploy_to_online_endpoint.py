...
if __name__ == "__main__":
    main()
afrom azure.identity import DefaultAzureCredential
from azure.ai.ml import MLClient
...
def create_or_update_deployment(ml_client, endpoint_name, deployment_name):
    ...
    print(f"Using registered model: {latest_model.name} version {latest_model.version}")

    deployment = ManagedOnlineDeployment(
        name=deployment_name,
        endpoint_name=endpoint_name,
        model=latest_model.id,    # ← Use registered model ID
        instance_type="Standard_D2as_v4",
        instance_count=1,
    )
    return ml_client.online_deployments.begin_create_or_update(deployment).result()
